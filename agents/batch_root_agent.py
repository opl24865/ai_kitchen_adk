...
from agents.batch_resource_assessment_agent import (
...
batch_root_agent = SequentialAgent(
    ...
    description=(
        ...
        "It handles multi-order intake, resource assessment, "
        "batch planning, sequential execution, and notification "
        "using ADK SequentialAgent."
    ),
    sub_agents=[
...
# Google ADK 會從應用程式模組尋找名為 root_agent 的入口。
# main.py 則使用語意更明確的 batch_root_agent。
